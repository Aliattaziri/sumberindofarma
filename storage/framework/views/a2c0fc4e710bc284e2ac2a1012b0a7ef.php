        ...
        <?php if($latestNews->count() > 0): ?>
            <div style="display: flex; gap: 1rem; overflow-x: auto; overflow-y: hidden; white-space: nowrap; padding-bottom: 0.75rem; scroll-snap-type: x proximity; -ms-overflow-style: none; scrollbar-width: none;" class="news-scroll">
                ...
                <?php $__currentLoopData = $latestNews; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $news): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <a href="<?php echo e(route('news.index', ['news_id' => $news->id])); ?>" class="news-card" style="text-decoration: none; color: inherit; display: block; min-width: 0; border-radius: 1.15rem; overflow: hidden; background: rgba(17,24,39,0.8); border: 1px solid rgba(255,255,255,0.08); box-shadow: 0 10px 30px rgba(0,0,0,0.25); transition: transform 0.2s ease, border-color 0.2s ease; scroll-snap-align: start;">
                        <div class="news-media" style="position: relative; width: 100%; aspect-ratio: <?php echo e($news->ratio === '9:16' ? '9 / 16' : '3 / 4'); ?>; background: #111827; overflow: hidden;">
                            <?php if($news->file): ?>
